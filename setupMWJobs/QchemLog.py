...
class QchemLog:
    ...
    def loadKro(self):
        """
        Load the 1D external active rotor. This, by default assumes that all 
        molecules are treated as symmetric tops. Thus, a 1D external rotor is identified via 
        I_1D,ext != Ib ~ Ic (assuming two external moments of enertia are alike, the third, 
        unique). Loads the kro in units of wavenumber (cm^-1), although the Qchem output
        is in units of amu-Bohr^2, and converted into cm^-1 (k is from high to low)
        """

        f = open(self.path, 'r')
        line = f.readline()
        while line != '':
            # Locate the moments of inertia
            if 'Principal axes and moments of inertia in amu*Bohr^2' in line:
                line = f.readline()
                line = f.readline()
                k = [float(k) for k in line.split()[2:]]
                # Convert to cm^-1
                k = [16.45/i*1.88987**2 for i in k]
                if abs(k[0]-k[1]) < abs(k[2]-k[1]):
                    kro = k[2]
                else:
                    kro = k[0]
                #Read the next line    
...
#        return Conformer(E0=(E0*0.001,"kJ/mol"), modes=modes, spinMultiplicity=spinMultiplicity, opticalIsomers=opticalIsomers)
        return freq              
    def loadFreq(self, symmetry=None, spinMultiplicity=None, opticalIsomers=1):
        """
        Load the molecular degree of freedom data from a log file created as
        the result of a Qchem "Freq"  calculation. As
        Qchem's guess of the external symmetry number is not always correct,
